...
public class TTTAndroidActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		buttons[0] = (RadioButton)findViewById(R.id.pieceButton);
		buttons[1] = (RadioButton)findViewById(R.id.tornadoButton);
		buttons[2] = (RadioButton)findViewById(R.id.cancelButton);
		...
		rules.setOnClickListener(new OnClickListener(){
			...
			public void onClick(View v)
			{
				Intent instr = new Intent(TTTAndroidActivity.this, InstructionsActivity.class);
				startActivity(instr);
			}
		});
		...
		quit.setOnClickListener(new OnClickListener(){
			...
			public void onClick(View v)
			{
				TTTAndroidActivity.this.finish();
			}
		});
		aiInProgress = true;
			...
			for (int j = 0; j < 7; j++)
			{
				...
				img.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v)
					{
						...
						int type = 0;
						if (buttons[1].isChecked()) type = 1;
						if (buttons[2].isChecked()) type = 2;
						int[] move = {type,x,y};
	...
	private int[] max(HashMap<Float,int[]> map)
	{
		...
		Util.debug("TTT", "max value is " + maxValue);
		return result;
	}
	/**
	...
	private boolean playMove(int[] move)
	{
		...
		if (result != Board.NO_ONE) //don't need to worry about invalid
		{
			...
			inGame = false;
			//make game-specific UI elements invisible (8 is invisible and no size)
			buttons[0].setVisibility(8);
			buttons[1].setVisibility(8);
			buttons[2].setVisibility(8);
	 ...
	 * @param id 0 1 2 3 or 4 for player stupid ai easy ai medium ai or hard ai
	 */
	private void allowPlay(int id)
	{
		if (id == 0) //Player's move
		{
			return; //on click listeners automatically handle player if !aiInProgress
		}
		aiInProgress = true; //Some ai's move, don't let player move
		if (id == 1) //Stupid ai
		{
			Thread aiThread = new Thread(new Runnable()
			{
	...
	private int[] hardAi(int maxDepth)
	{
		...
		//Sort possible moves by the board heuristic immediately after playing them;
		//alphabeta works faster if it can eliminate more moves immediately
		//which it can do if it starts out on a good tree and often the best
		//move now is the best move later b/c there are no strategic complexities
		//within the ai's range
		int[] move = new int[3];
		...
		for (int type = 0; type < 3; type++)
		{
			...
			for (int x = 0; x < 7; x++)
			{
				...
				for (int y = 0; y < 7; y++)
				{
					...
					{
						//set value to whatever the heuristic function gives
						value = heuristic(board, isXTurn);
					}
		...
		//Actually go through and test the moves, which are now in order of immediate effect 
		...
		for (MoveValue m : moveValues)
		{
			...
			float value = hardTestMove(move,boards, maxDepth, bestValue);
		...
		Util.debug("TTT", "bestValue is " + bestValue + ", bestMove is " + toString(bestMove));
		if (bestMove == null)
		{
			Util.warn("TTT", "bestMove in max is null");
		}
		return bestMove;
	}
	private Float hardTestMove(int[] move, ArrayList<Board> boardSet, int maxDepth, float initialAlpha)
	{
		Float result = hardTestMoveHelper(move,0,boardSet, maxDepth, -10.f, Float.MAX_VALUE, true);
		if (result == null)
		{
			Log.wtf("TTT", "alphabetaHardTestMoveHelper returned null");
			//don't use util here - if this is wrong, it should crash regardless of debug
		}
		return result;
	}
			...
			for (int type = 0; type < 3; type++)
			{
				...
				for (int x = 0; x < 7; x++)
				{
					...
					for (int y = 0; y < 7; y++)
					{
						...
						float nextlevel = hardTestMoveHelper(newmove, depth+1, boardSet, maxDepth, alpha, beta, !isMaximizing);
...
//			if (beta < 0.05f) return 0.999f-beta; //winning sooner is better
	...
	private void hardAiMove()
	{
		...
		stalemateTimeSum = 0;
		int[] move = new int[3];
		while ((SystemClock.uptimeMillis()-startTime) <= 250)
		{
		...
		//Benchmarking, not needed right now
...
//		Util.debug("TTT", "Stalemate took " + stalemateTimeSum + " ms total");
	...
	private int[] mediumAi(int maxDepth)
	{
		ArrayList<Board> boards = new ArrayList<Board>();
		boards.add(board.copy());
		boards.add(board.copy());
		...
		for (int type = 0; type < 3; type++)
		{
			...
			for (int x = 0; x < 7; x++)
			{
				...
				for (int y = 0; y < 7; y++)
				{
					move[2] = y;
					boards.get(0).set(board);
					float value = mediumTestMove(move,boards, maxDepth);
					//bestValue in that parameter list is used as the starting alpha value
					//for the alphabeta recursion, thus cutting off branches based on level 0
					if (value > bestValue)
					{
	...
	private void mediumAiMove()
	{
		...
		int depth = 1;
		int[] move = new int[3];
		while ((SystemClock.uptimeMillis()-startTime) <= 350)
		{
			move = mediumAi(depth); //If the ai can run the level below in a certain time
			//run the ai to the next depth. Not optimized but memory-light.
			depth++;
		}
		if (move == null)
		{
			Log.wtf("TTT", "move is null at mediumAiMove");
		}
		//thread stuff
		Message mes = aiHandler.obtainMessage(0, move);
		mes.sendToTarget();
	...
	private Float mediumTestMove(int[] move, ArrayList<Board> boardSet, int maxDepth)
	{
		return mediumTestMoveHelper(move,0,boardSet, maxDepth);
	}
	//like testMove but it is given the depth searched up to, so it searches to the (4-depth)th move.
	//assumes that no one has won yet on b
	private Float mediumTestMoveHelper(int[] move, int depth, ArrayList<Board> boardSet, int maxDepth)
	{
		if (boardSet.size() <= depth+1) boardSet.add(board.copy());
		Board b = boardSet.get(depth);
		boolean turn = b.isXTurn; //the original turn so that play does not make it play for the wrong side
		...
		for (int type = 0; type < 3; type++)
			for (int x = 0; x < 7; x++)
				for (int y = 0; y < 7; y++)
				{
					int[] newmove = {type,x,y};
					boardSet.get(depth+1).set(b);
					worst = Math.max(worst, mediumTestMoveHelper(newmove, depth+1, boardSet, maxDepth));
					//The value of a move is 1 minus the worst case scenario that can result.
					//The enemy's move being high is bad so this finds worst case.
				}
	...
	private static float heuristic(Board b, boolean isXTurn)
	{
		float friendly = 0.f;
		float enemy = 0.f;
		for (int i = 0; i < 7; i++)
		...
		enemy+=(5*enemybonus);
		float value = friendly / (friendly+enemy);
		return value;
	}
}